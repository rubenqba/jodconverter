		...
		try {
			...
			for (File inputFile : files) {
				String inputExtension = FilenameUtils.getExtension(inputFile
				        .getName());
				DocumentFormat inputFormat = formatRegistry
				        .getFormatByExtension(inputExtension);
				assertNotNull("unknown input format: " + inputExtension,
				        inputFormat);
				Set<DocumentFormat> outputFormats = formatRegistry
				...
				for (DocumentFormat outputFormat : outputFormats) {
					...
					// TODO use file detection to make sure outputFile is in the
					// expected format
				}
			}
		} finally {
