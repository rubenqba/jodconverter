...
class OfficeProcess {
	...
	private void prepareInstanceProfileDir() throws OfficeException {
		if (instanceProfileDir.exists()) {
			log.warn(String.format("profile dir '%s' already exists; deleting",
			        instanceProfileDir));
			deleteProfileDir();
		}
	...
	private void addBasisAndUrePaths(ProcessBuilder processBuilder)
		...
		if (!basisLink.isFile()) {
			log.info("no %OFFICE_HOME%/basis-link found; assuming it's OOo 2.x and we don't need to append URE and Basic paths");
			return;
		}
		...
		log.info(String.format("setting %s to \"%s\"", pathKey, path));
	...
	public int forciblyTerminate(long retryInterval, long retryTimeout)
		...
		log.info(String.format("trying to forcibly terminate process: '"
		        + unoUrl + "'"
		        + (pid != PID_UNKNOWN ? " (pid " + pid + ")" : "")));
